...
ostream& operator << (ostream& os, const Transcript& tran) {
	os << left << setw(10) << tran.getId() << setw(15) << tran.getStudent().getId()
		...
		<< setw(20) << tran.getSubject().getName() << setw(10) << tran.getGradeLevel1()
...
void Transcript::calculateGPA() {
	gpa = 0.1f * gradeLevel1 + 0.2f * gradeLevel2 + 0.3f * gradeLevel3 + 0.4f * gradeLevel4;
	string currentRank = "";
	if (gpa >= 9.0f) {
		currentRank = "A+";
	}
...
void Transcript::setStudent(const Student& student) { this->student = student; }

void Transcript::setSubject(const Subject& subject) { this->subject = subject; }
