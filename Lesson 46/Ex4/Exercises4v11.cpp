...
bool updateMark(Student students[], const int numOfStudent, string key, float newMark) {
	key = tolower(key);
	for (int i = 0; i < numOfStudent; i++)
	{
		if (key.compare(tolower(students[i].studentId)) == 0) {
			students[i].mark.cPlusplus = newMark;
			return true; // update success
		}
	}
	cout << "Khong tim thay sinh vien co ma \"" << key << "\".\n";
	return false; // failed
}
	...
	do
	{
		...
		case 11:
			if (numOfStudent > 0) {
				...
				cout << "Nhap diem moi: ";
				cin >> newMark;
				auto updateResult = updateMark(students, numOfStudent, key, newMark);
				if (updateResult) {
					cout << "==> Cap nhat diem thanh cong!\n";
				}
				else {
					cout << "==> Cap nhat diem that bai!\n";
				}
			}
			else {
				cout << "==> Danh sach sinh vien rong. <==" << endl;
